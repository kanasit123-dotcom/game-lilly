...
- Speech is pre-recorded (2026-09-19, the user chose these voices after listening to demos): `design/voice.py` (needs `pip install edge-tts`) collects every string literal in `js/` (constant parts of template literals too), renders Thai with Microsoft Neural `th-TH-PremwadeeNeural` and English with `en-US-AnaNeural` into `assets/voice/<th|en>/<md5>.mp3`, writes each `manifest.json` (text → file; English keys lowercase) and fills the `// voice-start … // voice-end` list in `sw.js`. It also records the numbers 0–100 (Thai) and 0–20 (English). Re-run it after adding or changing any spoken text, then bump the cache version. `clipsFor(text, lang)` stitches composed sentences ("5 บวก 3 เท่ากับ 8", "ทำแพนเค้ก") from the longest matching phrases left to right (single-character keys only at word boundaries); when a sentence cannot be covered, or a clip fails, `speak()` falls back to the device voice. Decoded clips are silence-trimmed (`trimmed()`: edge-tts pads every file to ~1.9 s while a word lasts ~0.5 s) and scheduled back-to-back with a 90 ms gap, so a stitched sum takes ~3 s instead of ~9 s. Sentences that are composed at runtime sound choppy word by word, so `voice.py` also renders longer phrase chunks for known templates (word problems: "ลิลลี่มี<item>", "<n> <classifier>", "แม่ให้อีก", "รวมกันมีกี่<classifier>"…) — add chunks there whenever a new template is spoken. Clips the TTS refuses (single rare letters, Thai digits) are dropped from the manifest; Thai digits are mapped to Arabic in `spokenForm()`. Clips are decoded into the same Web Audio context as the effects — never an `<audio>` element, which switches iOS into media mode and silences Web Audio. Use `stopSpeech()` (not `speechSynthesis.cancel()`) to interrupt. `spokenForm()` turns `+ − = × ÷` (and a hyphen between digits) into words before matching or TTS, so "4 + 5 = ?" is read "สี่ บวก ห้า เท่ากับ".
- First install shows a progress overlay (`.first-load` in `js/main.js`): `sw.js` caches the shell in chunks of 25 and posts `install-progress` to uncontrolled clients. It only appears when there is no controller yet; version updates install silently behind the running game.
