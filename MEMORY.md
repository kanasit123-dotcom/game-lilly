...
- Service worker cache is currently `lilly-world-v26`. Bump the version for the next deployed release when shell files change.
...
- New always-unlocked playroom mini-game `writing` (`js/mini/writing.js`) lets Lilly freely write Thai consonants, English uppercase, and English lowercase. It has sample/no-sample modes, a reference-style box with two horizontal guide lines, spoken audio for every letter, no countdown timer, and manual "เสร็จแล้ว" completion; it does not reuse or alter the guided trace game.
